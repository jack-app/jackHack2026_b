...
export default function Board({ mapData, myPlayer, otherPlayers, switches }: BoardProps) {
    ...
    <div className="relative inline-block">
          ...
          row.map((cell, x) => {
            ...
            return (
              <Tile
                key={`${x}-${y}`}
                cell={cell}
                switchState={switchId && switches ? switches[switchId] : undefined}
                switchWeight={switchId ? mapData.switch_weights[switchId] : undefined}
              />
            );
          })
        )}
      ...
      {otherPlayers.map((player, i) => (
        <div
          ...
          style={{ left: player.x * TILE_SIZE, top: player.y * TILE_SIZE, width: TILE_SIZE, height: TILE_SIZE }}
        >
      ...
      <div
        className="absolute transition-all duration-150 pointer-events-none flex items-center justify-center"
        style={{ left: myPlayer.x * TILE_SIZE, top: myPlayer.y * TILE_SIZE, width: TILE_SIZE, height: TILE_SIZE }}
      >
