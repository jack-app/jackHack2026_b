...
export default function WaitingText({ isHost, onStartGame }: WaitingTextProps) {
  return (
    <div className="flex flex-col items-center justify-center space-y-12 text-white bg-slate-950 p-10 rounded-xl border border-white/10">
      {/* 待機メッセージセクション */}
      ...
      <div className="text-cyan-200 font-bold">I T E M S</div>
      ...
      <div className="flex flex-col items-center space-y-10 relative">
        {ITEMS.map((item) => (
          <div key={item.id} className="relative group">
            {/* アイテム画像スロット */}
            <div
              className={`w-20 h-20 bg-black border-2 ${item.color} ${item.shadow} flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_25px_rgba(255,255,255,0.3)] cursor-help`}
            >
              <Image
                src={item.imageSrc}
                alt={item.name}
            ...
            {/* アイテム名 */}
            <p
              className={`text-center mt-2 text-xs font-bold tracking-widest uppercase opacity-70 group-hover:opacity-100 transition-opacity`}
            >
              {item.name}
            </p>
            ...
            {/* 赤い六角形吹き出し */}
            <div
              className={`absolute ${item.pos} top-0 opacity-0 scale-75 translate-y-4 pointer-events-none transition-all duration-300 group-hover:opacity-100 group-hover:scale-100 group-hover:translate-y-0 z-50`}
            >
              <div
